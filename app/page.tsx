...
export default async function Home() {
  ...
  const ciLabel = metrics?.ciStatus === "completed" ? (metrics.ciConclusion === "success" ? "Passing" : "Failing") : metrics?.ciStatus ?? "Unknown";
    ...
    <main className="shell">
      ...
      <section className="content" id="overview">
        ...
        <section className="hero card">
          <div><p className="eyebrow">Current status</p><h2>{currentStage}</h2><p className="muted wide">Progress is calculated from the state of tracked workflow issues in the GitHub repository. No percentage is manually hard-coded.</p></div>
        ...
        <section className="metrics" id="activity">
          <Metric label="Commits" value={metrics?.commits ?? "—"} /><Metric label="Pull requests" value={metrics?.pullRequests ?? "—"} /><Metric label="Open issues" value={metrics?.openIssues ?? "—"} /><Metric label="CI runs" value={metrics?.workflowRuns ?? "—"} />
        </section>
        ...
        <section className="columns" id="features">
          <div className="card panel"><div className="section-head"><div><p className="kicker">SCOPE</p><h2>Product roadmap</h2></div></div><div className="roadmap"><Row title="Project foundation" status="Tracked" /><Row title="Live GitHub metrics" status="Operational" /><Row title="Workflow & milestones" status="GitHub issues" /><Row title="Feature tracking" status="Next" /><Row title="Deployment intelligence" status="Planned" /></div></div>
          <div className="card panel" id="deployment"><p className="kicker">REPOSITORY HEALTH</p><h2>GitHub & delivery</h2><div className="health"><div><span className="health-dot" /> GitHub connected</div><strong>{metrics ? "Operational" : "Unavailable"}</strong></div><div className="details"><span>Repository</span><b>A3-Creativz</b><span>Branch</span><b>main</b><span>CI status</span><b>{ciLabel}</b><span>Deployments</span><b>{metrics?.deploymentCount ?? "—"}</b><span>Last push</span><b>{metrics?.lastPush ? new Date(metrics.lastPush).toLocaleString() : "—"}</b></div></div>
        </section>
